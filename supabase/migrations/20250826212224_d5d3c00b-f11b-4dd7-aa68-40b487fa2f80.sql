-- Atualizar função para corrigir fluxo de cards reservados e traduzir mensagens
...
BEGIN
  ...
  -- CASO ESPECIAL: Card reservado para o mesmo email (pular verificação de código)
  ...
  -- Validar formato do código de desbloqueio (deve ter 6 dígitos) - apenas se necessário
  IF card_status_val != 'reserved' OR existing_email != clean_email THEN
  ...
  -- Verificar se o card está disponível ou reservado por outro usuário
  IF card_status_val = 'available' THEN
    -- Card disponível, prosseguir com desbloqueio
    NULL;
  ELSIF card_status_val = 'reserved' THEN
    -- Verificar se reservado por outro email e ainda dentro do período
