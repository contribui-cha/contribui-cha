-- Fix the verify_unlock_code_and_reveal function to properly handle reserved cards
-- Move the reserved card check BEFORE code validation
...
BEGIN
  ...
  -- PRIORIDADE MÁXIMA: Card reservado para o mesmo email (sucesso IMEDIATO)
  IF card_status_val = 'reserved' AND existing_email = clean_email THEN
    ...
    IF reserved_until_val IS NOT NULL AND reserved_until_val > now() THEN
      -- Sucesso automático - card já reservado para este email, SEM validar código
